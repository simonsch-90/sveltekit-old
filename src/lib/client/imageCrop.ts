import type { Point, Size, ImageSize } from '$lib/types';
 ...
 */
export const getCropSize = (imgWidth: number, imgHeight: number, aspect: number) => {
 ...
 * @param shouldRound
 */
const limitArea = (max: number, value: number, shouldRound = false) => {
	const v = shouldRound ? Math.round(value) : value;
	return Math.min(max, Math.max(0, v));
};

const noOp = (max: number, value: number) => {
	return value;
};
 ...
 * @param b
 */
export const getCenter = (a: Point, b: Point) => ({
	x: (b.x + a.x) / 2,
	y: (b.y + a.y) / 2,
});
